...
class SupplyFan(object):
    def fan_should_start(self):
        self.wait_for_state(
            self.controller["STARTSTOP-STATE"], "On", callback=self.fan_is_on
        )
...
class ZNT_State(object):
    def to_heating(self, sensor="ZN-T", setpoint="EFFHTG-SP", delta=5):
        ...
        self.wait_for_state(
            self.controller["ZNT-STATE"],
            "Heating",
            timeout=600,
            callback=self.dat_is_heating,
        )
    ...
    def to_cooling(self, sensor="ZN-T", setpoint="EFFCLG-SP", delta=5):
        ...
        self.wait_for_state(
            ...
            "Cooling",
            timeout=600,
            callback=self.dat_is_cooling,
        )

    def to_satisfied(self, sensor="ZN-T", htg_sp="EFFHTG-SP", clg_sp="EFFCLG-SP"):
        self.note("switching to satisfied")
        sp = (
            (self.controller[clg_sp].value - self.controller[htg_sp].value) / 2
        ) + self.controller[htg_sp].value
        self.controller[sensor].write(sp)
        self.wait_for_state(
            self.controller["ZNT-STATE"],
            "Satisfied",
            timeout=600,
            callback=self.dat_is_off,
        )
...
class Reheat(object):
    def reheat_should_stop(self):
        ...
        self.wait_for_state(
            self.controller["RH-OUTSTATE"], "Off", callback=self.reheat_is_off
        )
    ...
    def reheat_should_start(self):
        ...
        self.wait_for_state(
            self.controller["RH-OUTSTATE"],
            "T Control",
            timeout=300,
            callback=self.reheat_in_control,
        )
        # self.controller['DA-T'].write(old_value)
        # self.note('Reverting to old DA-T value')
...
class Dampers(object):
    def dampers_should_close(self):
        self.wait_for_state(
            self.controller["MAD-OUTSTATE"], "Close", callback=self.dampers_closed
        )
    ...
    def dampers_should_modulate(self):
        ...
        self.wait_for_state(
            self.controller["MAD-OUTSTATE"],
            "T Control",
            callback=self.dampers_are_modulating,
        )
        # self.controller['DA-T'].write(old_value)
        # self.note('Reverting to old DA-T value')
    ...
    def dampers_are_modulating(self):
        if "MAD-O" in self.controller:
            self.wait_for_value_gt(
                self.controller["MAD-O"], self.controller["OAD-MINPOS"]
            )
        if "OAD-O" in self.controller:
            self.wait_for_value_gt(
                self.controller["OAD-O"], self.controller["OAD-MINPOS"]
            )
    ...
    def dampers_should_open_to_minimum(self):
        self.wait_for_state(
            self.controller["MAD-OUTSTATE"],
            "Ramp Min OA",
            callback=self.dampers_are_at_min_pos,
        )

    def dampers_are_at_min_pos(self):
        if "MAD-O" in self.controller:
            self.wait_for_state_not(
                self.controller["MAD-O"], self.controller["OAD-MINPOS"].value
            )
        if "OAD-O" in self.controller:
            self.wait_for_state_not(
                self.controller["OAD-O"], self.controller["OAD-MINPOS"].value
            )
        if "RAD-O" in self.controller:
...
class AHU_Cooling(object):
    def ahu_cooling_should_stop(self):
        self.wait_for_state(
            self.controller["CLG-OUTSTATE"], "Off", callback=self.ahu_cooling_is_off
        )
    ...
    def ahu_cooling_should_start(self):
        ...
        self.wait_for_state(
            self.controller["CLG-OUTSTATE"],
            "T Control",
            callback=self.ahu_cooling_in_control,
        )
    ...
    def ahu_cooling_dehumidif(self):
        self.wait_for_state(
            self.controller["CLG-OUTSTATE"],
            "H Control",
            callback=self.ahu_cooling_is_off,
        )
...
class Relief_Fan(object):
    def rlf_fan_should_stop(self):
        self.wait_for_state(
            self.controller["RLF-OUTSTATE"], "Off", callback=self.rlf_fan_is_off
        )
    ...
    def rlf_fan_should_modulate(self):
        self.wait_for_state(
            self.controller["RLF-O"],
            "BS-P Control",
            callback=self.rlf_fan_is_modulating,
        )
